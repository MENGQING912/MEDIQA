...
class ChallengeDataset(Minigpt4QwenDataset, __DisplMixin):
    def __init__(self, vis_processor, text_processor, vis_root, ann_paths):
        self.vis_root = vis_root
    ...
    def __getitem__(self, index):
        ...
        image_path = os.path.join(self.vis_root,ann['image_ids'][0])
        image = Image.open(image_path).convert("RGB")

        image = self.vis_processor(image)
        instruction = self.text_processor('<Img><ImageHere></Img> '+ann['query_title_en']+ann['query_content_en'])

        # conversations = {"from": "user", "value":instruction}
